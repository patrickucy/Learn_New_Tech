...
- (void) tableView:(UITableView *)tableView accessoryButtonTappedForRowWithIndexPath:(NSIndexPath *)indexPath{
    ...
    TRArea * selectedArea = self.area.subAreas[indexPath.row];
    
    //按钮触发的抛回  （出事了）
    //按钮 -> 
    [self.delegate areaViewController:self
                        didSelectArea:selectedArea];
    
    [self dismissViewControllerAnimated:YES completion:nil];
}

//接住消息 再抛出去（自己处理不了的交给下一个对象处理）
// -> 接住
- (void)areaViewController:(TRAreaViewController *)areaViewController didSelectArea:(TRArea *)area{
    
    // 不能处理 ->
    [self.delegate areaViewController:self didSelectArea:area];
